...
class Hash
  def deep_diff(hash)
    (keys | hash.keys).each_with_object({}) do |key, diff|
      next if self[key] == hash[key]
      diff[key] = if self[key].is_a?(Hash) && hash[key].is_a?(Hash)
                    self[key].deep_diff(hash[key])
                  else
                    [self[key], hash[key]]
                  end
    end
  ...
  def deep_merge(other_hash, &merger)
    merger ||= proc do |_key, oldval, newval|
      begin
        oldval.deep_merge(newval, &merger)
      rescue
        newval
      end
    end
...
module Graphiti
  class << self
    def data_bag_item(data_bag, item, missing_ok = false)
      raw_hash = data_bag_item(data_bag, item)
  ...
  class Dashboard < Common
    class << self
      ...
      def create(options = {})
        raise ArgumentError, 'Must pass a hash' unless options.respond_to? :has_key?
      ...
      def delete(slug)
        ...
        raise ArgumentError, "#{slug} does not exist, not deleting" unless exists? slug
  ...
  class Graph < Common
    ...
    def save
      if remote_graph && remote_graph.has_key?('uuid')
    ...
    def targets_and_options
      if Template.graphs(node).has_key? type
        Template.graph_for(type, default_options, node)
      else
        raise StandardError, "#{self}#targets: no template defined for #{type}"
      end
    end
